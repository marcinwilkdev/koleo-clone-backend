"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.setData = exports.signup = exports.signin = void 0;
var WebTokenService_1 = __importDefault(require("../services/other/WebTokenService"));
var EncryptionService_1 = __importDefault(require("../services/other/EncryptionService"));
var UserService_1 = __importDefault(require("../services/database/UserService"));
var helpers_1 = require("../util/helpers");
var HttpException_1 = __importDefault(require("../util/HttpException"));
var generateUserData = function (user) {
    var userData = user.email;
    if (user.firstName) {
        userData = user.firstName;
    }
    return userData;
};
var signin = function (req, res, next) { return __awaiter(void 0, void 0, void 0, function () {
    var body, email, password, user, isPasswordEqual, token, userData, responseBody, err_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                body = req.body;
                email = body.email;
                password = body.password;
                _a.label = 1;
            case 1:
                _a.trys.push([1, 4, , 5]);
                return [4 /*yield*/, UserService_1.default.getInstance().findByEmail(email)];
            case 2:
                user = _a.sent();
                if (!user) {
                    throw new HttpException_1.default("User not found.", 404);
                }
                return [4 /*yield*/, EncryptionService_1.default.getInstance().compare(password, user.password)];
            case 3:
                isPasswordEqual = _a.sent();
                if (!isPasswordEqual) {
                    throw new HttpException_1.default("Wrong password.", 401);
                }
                token = WebTokenService_1.default.getInstance().sign({ userId: user.id }, 1);
                userData = generateUserData(user);
                responseBody = {
                    message: "Logged in succesfully.",
                    token: token,
                    userData: userData,
                };
                res.status(200).json(responseBody);
                return [3 /*break*/, 5];
            case 4:
                err_1 = _a.sent();
                helpers_1.handleErrors(err_1, next);
                return [2 /*return*/];
            case 5: return [2 /*return*/];
        }
    });
}); };
exports.signin = signin;
var signup = function (req, res, next) { return __awaiter(void 0, void 0, void 0, function () {
    var body, email, hashedPassword, user, savedUser, token, userData, responseBody, err_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 3, , 4]);
                helpers_1.validateRequest(req);
                body = req.body;
                email = body.email;
                return [4 /*yield*/, EncryptionService_1.default.getInstance().hash(body.password, 12)];
            case 1:
                hashedPassword = _a.sent();
                user = {
                    email: email,
                    password: hashedPassword,
                };
                return [4 /*yield*/, UserService_1.default.getInstance().save(user)];
            case 2:
                savedUser = _a.sent();
                token = WebTokenService_1.default.getInstance().sign({ userId: savedUser.id }, 1);
                userData = generateUserData(savedUser);
                responseBody = {
                    message: "User created succesfully.",
                    token: token,
                    userData: userData,
                };
                res.status(201).json(responseBody);
                return [3 /*break*/, 4];
            case 3:
                err_2 = _a.sent();
                helpers_1.handleErrors(err_2, next);
                return [2 /*return*/];
            case 4: return [2 /*return*/];
        }
    });
}); };
exports.signup = signup;
var setData = function (req, res, next) { return __awaiter(void 0, void 0, void 0, function () {
    var body, userId, user, savedUser, userData, responseBody, err_3;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                body = req.body;
                userId = req.userId;
                _a.label = 1;
            case 1:
                _a.trys.push([1, 4, , 5]);
                return [4 /*yield*/, UserService_1.default.getInstance().findById(userId)];
            case 2:
                user = _a.sent();
                if (!user) {
                    throw new HttpException_1.default("User not found.", 404);
                }
                user.discount = body.discount;
                user.firstName = body.firstName;
                user.lastName = body.lastName;
                user.dateOfBirth = body.dateOfBirth;
                return [4 /*yield*/, UserService_1.default.getInstance().update(user)];
            case 3:
                savedUser = _a.sent();
                userData = generateUserData(savedUser);
                responseBody = {
                    message: "User data set succesfully.",
                    userData: userData,
                };
                res.status(200).json(responseBody);
                return [3 /*break*/, 5];
            case 4:
                err_3 = _a.sent();
                helpers_1.handleErrors(err_3, next);
                return [2 /*return*/];
            case 5: return [2 /*return*/];
        }
    });
}); };
exports.setData = setData;
